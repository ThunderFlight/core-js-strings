...
function isPalindrome(str) {
  ...
  return (
    splittedString
      ...
      .reverse()
      .join('')
      .toLowerCase() === splittedString.join('')
  );
}
...
function extractNameFromTemplate(value) {
  const splittedSentence = value.split(' ');
  const firstName = splittedSentence[1];
  const lastName = splittedSentence[2].slice(0, splittedSentence[2].length - 1);
...
function encodeToRot13(str) {
  ...
  return splitedString
    .map((letter) => {
      ...
      const toRot13 = charCode + 13;
      if (toRot13 > 122) {
        return String.fromCharCode(toRot13 - 122 + 96);
      }
      if (toRot13 > 90 && letter === letter.toUpperCase()) {
        return String.fromCharCode(toRot13 - 90 + 64);
      }
      return String.fromCharCode(toRot13);
    })
    .join('');
}
